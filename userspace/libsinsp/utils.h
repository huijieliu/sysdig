...
template <class M>
class sinsp_scoped_lock
{
public:
	sinsp_scoped_lock() = delete;
	sinsp_scoped_lock(const sinsp_scoped_lock&) = delete;
	sinsp_scoped_lock& operator = (const sinsp_scoped_lock&) = delete;

	explicit sinsp_scoped_lock(M& mutex): m_mutex(mutex)
	{
		m_mutex.lock();
	}

	sinsp_scoped_lock(M& mutex, long milliseconds): m_mutex(mutex)
	{
	...
	~sinsp_scoped_lock()
	{
		...
		{
			sinsp_unexpected(__FILE__, __LINE__);
		}
	}
...
template <class M>
class sinsp_scoped_lock_with_unlock
{
public:
	sinsp_scoped_lock_with_unlock() = delete;
	sinsp_scoped_lock_with_unlock(const sinsp_scoped_lock_with_unlock&) = delete;
	sinsp_scoped_lock_with_unlock& operator = (const sinsp_scoped_lock_with_unlock&) = delete;

	explicit sinsp_scoped_lock_with_unlock(M& mutex): m_pmutex(&mutex)
	{
		m_pmutex->lock();
	}

	sinsp_scoped_lock_with_unlock(M& mutex, long milliseconds): m_pmutex(&mutex)
	{
		m_pmutex->lock(milliseconds);
	}

	~sinsp_scoped_lock_with_unlock()
	{
		try
		{
			unlock();
...
public:
	enum mutex_type_t
	{
		SINSP_MUTEX_RECURSIVE,
		SINSP_MUTEX_NONRECURSIVE
	};

	typedef sinsp_scoped_lock<sinsp_mutex> scoped_lock;

	sinsp_mutex(mutex_type_t type = SINSP_MUTEX_RECURSIVE);
	~sinsp_mutex();

	void lock();
	bool try_lock();
	bool try_lock(long milliseconds);
	void unlock();
...
class sinsp_fast_mutex : public sinsp_mutex
{
public:
	typedef sinsp_scoped_lock<sinsp_fast_mutex> scoped_lock;
